...
async def test_connection_path_selection_tolerates_none_rssi() -> None:
    ...
    class NoneRssiScanner(FakeScanner):
        """Scanner whose discovery view reports no signal strength."""

        def get_discovered_device_advertisement_data(
            self, address: str
        ) -> tuple[BLEDevice, AdvertisementData] | None:
            """Return the discovered device with an rssi-less advertisement."""
            if (
                device_adv := super().get_discovered_device_advertisement_data(address)
            ) is None:
                return None
            device, adv = device_adv
            return device, adv._replace(rssi=None)

    scanner1 = NoneRssiScanner("scanner1", "Scanner 1", connector1, True)
    ...
    scanner1.inject_advertisement(
        device1, generate_advertisement_data(local_name="Test Device", rssi=-70)
    )
    device2 = generate_ble_device(
